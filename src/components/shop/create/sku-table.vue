	...
	<table class="table table-sm table-bordered">
	  ...
	  <tbody v-if="skuLabels.length">
	    <tr v-for="(item,index) in list" :key="index">
		  <!-- 商品规格 -->
	      <th scope="row" v-for="(sku,skuI) in item.skus" :key="skuI" class="text-center">
		      <div v-if="skuLabels[skuI].type == 0">{{ sku.value }}</div>
		      <div v-else-if="skuLabels[skuI].type == 1">{{ sku.value }}</div>
		      <div v-else-if="skuLabels[skuI].type == 2">
				  <img :src="sku.value" style="width: 30px;height: 30px;">
			  </div>
		  </th>
