		...
		<el-table border class="mt-3"
		  ...
		  <el-table-column
			...
			<template slot-scope="scope">
				...
				<div class="media border-top py-2" v-for="(item,index) in scope.row.order_items" :key="index">
				  <img class="mr-3" style="width: 60px;height: 60px;" :src="item.goods_item ? item.goods_item.cover : ''">
				  <div class="media-body">
				    <p class="mt-0">
						<a class="text-primary">{{item.goods_item ? item.goods_item.title : '商品已被删除'}}</a>
					</p>
				  </div>
				</div>
			</template>
		  ...
		  <el-table-column
			...
			<template slot-scope="scope">
				...
				<el-button type="text" size="mini" v-if="scope.row.ship_status ==='pending' && scope.row.closed === 0 && scope.row.payment_method && scope.row.refund_status === 'applied'" @click="handleRefund(scope.row.id,1)">同意退款</el-button>
				<el-button type="text" size="mini" v-if="scope.row.ship_status ==='pending' && scope.row.closed === 0 && scope.row.payment_method && scope.row.refund_status === 'applied'" @click="handleRefund(scope.row.id,0)">拒绝退款</el-button>
			</template>
		  </el-table-column>
		</el-table>
