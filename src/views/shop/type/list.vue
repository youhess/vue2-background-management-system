...
<!-- 新增/修改模态框 -->
<el-dialog width="80%" title="添加类型" :visible.sync="createModel" top="5vh">
	<!-- 表单内容 -->
	<el-form :rules="rules" ref="form" :model="form" label-width="80px">
		...
		<el-form-item label="关联规格">
			<div class="d-flex">
				...
				style="line-height: initial;" v-for="(item,index) in form.sku_list"
				:key="index">
					<font>{{item.name}}</font>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 编辑属性
			editRow(scope){
				this.$set(scope.row,'isedit',!scope.row.isedit)
			},
			// 删除属性值
			delRow(index){
